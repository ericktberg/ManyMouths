...
export function LinkIngredientModal({ 
  isVisible, 
  ingredientId, 
  ingredientName,
  onClose, 
  onSuccess 
}: LinkIngredientModalProps) {
  ...
  const goodsQuery = useQuery({
    ...
    enabled: isVisible && currentStep === 'search'
  });
  ...
  const linkMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['ingredient-mappings'] });
      onSuccess?.();
      handleClose();
    }
  });
        ...
        <View className="flex-1 p-6">
          ...
          <View className="flex-row items-center justify-between mb-4">
            ...
            <Text className="text-xl font-bold">
              Link {ingredientName || 'Ingredient'}
            </Text>
